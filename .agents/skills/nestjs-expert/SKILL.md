---
name: nestjs-expert
description: "Nest.js framework expert specializing in module architecture, dependency injection, middleware, guards, interceptors, testing with Jest/Supertest, TypeORM/Mongoose/Prisma integration, and Passport.js auth..."
category: framework
risk: unknown
source: community
...
0. If a more specialized expert fits better, recommend switching and stop:
   ...
   - Prisma schema, migrations, or query optimization (without Nest wiring) → prisma-expert
...
### Database Integration (TypeORM, Mongoose & Prisma)
- Common issues: Connection management, entity/schema relationships, migrations
- Root causes: Incorrect configuration, missing decorators/directives, improper transaction handling
- Solution priority: 1) Fix configuration, 2) Correct entity/schema setup, 3) Implement transactions

**TypeORM**
- `@nestjs/typeorm`, entity decorators, repository pattern
- Resources: [TypeORM](https://docs.nestjs.com/techniques/database)

**Mongoose**
- `@nestjs/mongoose`, schema decorators, model injection
- Resources: [Mongoose](https://docs.nestjs.com/techniques/mongodb)

**Prisma**
- Prisma Client injection via custom provider; no `@nestjs/prisma` official package—use `PrismaService` pattern
- Schema in `prisma/schema.prisma`; migrations via `prisma migrate`; generated client for type-safe queries
- Common issues: PrismaService not provided globally, connection pool in serverless, N+1 with `include`/`select`
- Solution priority: 1) Provide PrismaService in a shared/global module, 2) Use `select`/`include` wisely, 3) Transactions via `prisma.$transaction()`
- Resources: [Prisma with Nest.js](https://docs.nestjs.com/recipes/prisma), [Prisma Docs](https://www.prisma.io/docs)
...
### 18. PrismaService not injectable / "Nest can't resolve dependencies" (Prisma)
**Frequency**: MEDIUM | **Complexity**: LOW
When using Prisma with Nest.js:
1. Create a dedicated PrismaService that extends PrismaClient and implements OnModuleInit/OnModuleDestroy
2. Register PrismaService in a module (e.g. PrismaModule) and export it; import that module where Prisma is needed
3. Use single instance: in serverless, reuse PrismaClient (e.g. globalThis) to avoid connection exhaustion
4. For schema/migrations: use prisma-expert skill; for DI/module wiring, apply the above in Nest
...
```typescript
// prisma.service.ts - injectable PrismaClient with lifecycle hooks
...
// prisma.module.ts
@Global() // optional: avoid importing in every module
@Module({ providers: [PrismaService], exports: [PrismaService] })
export class PrismaModule {}

// Usage in a service: inject PrismaService, use this.prisma.user.findMany(), etc.
```
...
### Database Integration (TypeORM, Mongoose, Prisma)
- [ ] **TypeORM**: Entity decorators use correct syntax (@Column() not @Column('description')); entities registered in TypeOrmModule.forFeature()
- [ ] **Mongoose**: Schemas and models registered in MongooseModule.forFeature()
- [ ] **Prisma**: PrismaService provided (e.g. global or shared module); schema validated with `prisma validate`; migrations via `migrate deploy` in production
- [ ] Connection errors don't crash the entire application
- [ ] Multiple database connections use named connections (TypeORM) or single Prisma datasource
- [ ] Database connections have proper error handling, retry logic, and graceful disconnect (Prisma: `$disconnect()`)
...
├─ Type safety & generated client? → Prisma (schema-first, migrations, great DX)
├─ Need migrations? → TypeORM or Prisma (Prisma: prisma migrate)
├─ Complex raw SQL / legacy schema? → TypeORM (flexible entity mapping)
├─ New greenfield app? → Prisma (recommended for type safety and tooling)
└─ Existing codebase with TypeORM/Mongoose? → Keep current ORM unless migrating
```
...
- [Prisma with Nest.js](https://docs.nestjs.com/recipes/prisma), [Prisma Documentation](https://www.prisma.io/docs)
